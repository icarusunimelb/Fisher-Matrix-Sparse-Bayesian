"""Various Hessian matrix/Fisher information matrix approximations."""
...
class Curvature(ABC):
    ...
    def _replace(sample: Tensor,
        ...
        """
        if bias is not None:
            # * here used to unpack containers
...
class Diagonal(Curvature):
    ...
    def update(self,
        ...
        if self.last_layer_mode:
            ...
            for layer in self.model.modules():
                pass
            self.update_layer(layer, batch_size)
        else:
            for layer in self.model.modules():
                self.update_layer(layer, batch_size)
    ...
    def sample(self,
        ...
        #Torch.tensor.normal_(mean=0,std=1): Fills self tensor with elements samples from the normal distribution parameterized by mean and std.
...
class KFAC(Curvature):
    ...
    def __init__(self,
        ...
        Args:
            model: Any (pre-trained) PyTorch model including all `torchvision` models.
        """
        super().__init__(model, layer_types, last_layer_mode)
        self.hooks = list()
        self.record = dict()
        ...
        for layer in model.modules():
            if layer.__class__.__name__ in self.layer_types:
                if layer.__class__.__name__ in ['Linear', 'Conv2d']:
                    ...
                    # The usage of hook is for saving intermediate variables
    ...
    def _save_output(self, module, grad_input, grad_output):
        # why * grad_output[0].size(0), check the paper
...
class EFB(Curvature):
    ...
    Source: https://arxiv.org/abs/1806.03884
    """
    def __init__(self,
                 model: Union[Module, Sequential],
...
class INF(Curvature):
    ...
    Source: https://arxiv.org/abs/2006.11631
    """
    def __init__(self,
                 model: Union[Module, Sequential],
